...
/**
 * Description of cls_txt_sql
 * Page-name of  cls_txt_sql.php
 *
...
class cls_txt_sql
{
    ...
    /**
     * 构造函数 指定txtSQL数据目录的路径
     * @param string $path
     * @return bool
     */
    private $_query = null;
    private $_data = null;
    private $_ERRORSPLAIN = array();
    private $_ERRORS  = array();

    public function __construct($path='')
    {
        if(empty ($path)){
            $this->_LIBPATH = PATH_TXTDB_DIR."/";
        }elseif(PATH_TXTDB_DIR == $path){
            $this->_LIBPATH = $path;
        }else{
            $this->_LIBPATH = PATH_TXTDB_DIR."/".$path;
        }
        return $this;
    }
	/**
    ...
    public function connect($user, $pass)
    {
        ...
        {
            return $this->_error(E_USER_ERROR, '指定无效的数据目录');
        }

        /* Instantiate parser and core class */
        $this->_query = new cls_txt_sql_core();
        ...
        {
            $this->_error(E_USER_WARNING, '数据库文件已损坏！');
            return FALSE;
        }
        $this->_data = $DATA;
        ...
        {
            $this->_error(E_USER_NOTICE, '用户访问被拒绝 \'' . $user . '\' (using password: ' . (!empty($pass) ? 'yes' : 'no') . ')');
            return FALSE;
        }
	 ...
	 * @access private
	 */
	private function _validate ($arguments)
	{
	 ...
	 * @access private
	 */
	protected function _error ($errno, $errstr, $errtype=NULL)
	{
	 ...
	 * @access private
	 */
	protected function _readFile ( $filename, $useCache = TRUE, $unserialize = TRUE )
	{
		if ( is_file($filename) )
		{
			if ( $useCache === TRUE )
	 ...
	 * @access private
	 */
	protected function _getColPos ($colname, $cols)
	{
	 ...
	 * @access private
	 */
	protected function _qsort($array, $num = 0, $order = "ASC", $left = 0, $right = -1)
	{
		if ( count($array) >= 1 )
		{
			if ( $right == -1 )
