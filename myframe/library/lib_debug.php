...
function debug_get_error_msg($log_type='debug', $errno, $errmsg, $filename, $linenum, $vars)
{
    ...
    //$err .= "<strong>MF框架应用错误跟踪：</strong><br />\n";
...
function debug_error_show()
{
    ...
    if( $_debug_error_msg != '' || $_debug_mt_info !='')
    {
        if (DEBUG_LEVEL === true || $debug_safe===true)
        {
            ...
            $js .= '}</script>';
            echo $js;
            echo '<div id="debug_ctl" style="width:100px;line-height:24px;position:absolute;top:5px;left:5px;border:1px solid #ccc; background: #FBFDE3; padding:2px;text-align:center">'."\n";
            ...
            echo '<div style="line-height:24px; background: #FBFEEF;;"><div style="float:left"><strong>MF框架应用错误/警告信息追踪：</strong></div><div style="float:right"><a href="#" onclick="javascript:debug_close_all();">[关闭全部]</a></div>';
